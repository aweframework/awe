...
The `TotpService.getQRCodeList(Boolean generate)` MUST call `enforceGenerateAccess()` before invoking secret generation when `generate=true`. Secret generation MUST be allowed only in two explicitly defined states: `pendingTotpEnrollment=true` (FORCE bootstrap) or `fullyAuthenticated=true` (settings activation). All other sessions — including enrolled users with `enabled2fa=true` who have not yet passed TOTP verification — MUST be denied with an explicit AWException before any secret rotation occurs. The XML `public="true"` flag is a transport-layer-only gate; it does NOT grant authorization for secret generation.
...
#### Scenario: Enrolled partially-authenticated session is denied secret generation via force-qr-code

- GIVEN a user has an existing 2FA secret (enabled2fa=true)
- AND the user has logged in but not yet passed TOTP verification (fullyAuthenticated=false)
- AND pendingTotpEnrollment=false
- WHEN getQRCodeList(true) is called (e.g., via the public force-qr-code query path)
- THEN the service MUST throw an explicit AWException before generateSecret() is called
- AND the user's existing secret MUST NOT be modified
- AND the XML public="true" alone MUST NOT be sufficient to authorize the operation

#### Scenario: Settings activation flow can regenerate the secret

- GIVEN a user is fully authenticated (fullyAuthenticated=true)
- AND is accessing the 2FA settings screen (private path, qr-code query)
- WHEN getQRCodeList(true) is called
- THEN the service MUST generate and store a new TOTP secret
- AND the session MUST remain fully authenticated
