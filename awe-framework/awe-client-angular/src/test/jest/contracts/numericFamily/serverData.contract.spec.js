...
describe("awe-framework/awe-client-angular/src/test/jest/contracts/numericFamily/serverData.contract.js", function () {
  let $injector;
  let $rootScope;
  let $compile;
  let $httpBackend;
  ...
  it("aggregates numeric payloads through ServerData.getFormValues preserving decimals and nulls", function () {
    ...
    jest.spyOn($storage, "get").mockImplementation(key => key === "api" ? createApiStorage("report", [amount, optional]) : {});
  ...
  it("preserves numeric zero in launchServerAction while merging target-specific fields", function () {
    ...
    amount.component.getSpecificFields = jest.fn().mockName("getSpecificFields").mockReturnValue({max: 25});
