...
describe('awe-framework/awe-client-angular/src/test/jest/services/grid/commons.js', function () {
  ...
  it('should initialize layers', function () {
    ...
    // Assert
    expect(spy.mock.calls.length).toEqual(7);
  ...
  it('should reposition save button', function () {
    // Mock
    let css = jest.fn().mockName('spy');
    let buttonPosition = jest.fn().mockName('buttonPosition');
    let component = getDefaultComponent();
    let commons = new GridCommons(component);
    commons.init();
    ...
    expect(buttonPosition.mock.calls.length).toEqual(1);
  });
  ...
  it('should delete the selected row (on GridComponents)', function () {
    ...
    let commons = new GridCommons(component);
    commons.init();
    jest.spyOn(component, "getColumns").mockReturnValue(["tutu", "lala"]);
