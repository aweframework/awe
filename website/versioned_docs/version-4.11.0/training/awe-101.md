...
This guide will show you step by step with the help of a video how to generate you first AWE web application. For this, we will use the [`awe-boot-angular-archetype`](https://search.maven.org/search?q=awe-boot-angular-archetype) archetype 
or the [`awe-boot-react-archetype`](https://search.maven.org/search?q=awe-boot-react-archetype) archetype.
...
:::info
You can use version **4.10.x** of AWE's archetype. Username is `test` and password is `test` for the test application.
:::
