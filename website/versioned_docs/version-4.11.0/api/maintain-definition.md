...
- Mark the auto‑increment column with `key="true"` in the `field` element so that AWE retrieves the generated id after the insert.
...
> Note: In `multiple` maintains, it is also mandatory to have a `field` with `key="true"` to indicate the table key. The same convention applies when the key is generated by the database.
...
<target name="insertWithKeySingle">
  ...
  <insert>
    <table id="tst_order"/>
    <field id="id" key="true" variable="OrderId"/>
