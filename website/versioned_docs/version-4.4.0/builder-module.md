---
id: builder title: Builder Module
---
...
This sample creates a client action which will try to navigate to the URL `http://go.to.another.url` if the current
screen is `specific-screen` (useful for dynamic-generated screens).
...
This sample search for the `.selector` CSS selector, and adds the "class1", "class2" and "class3" classes into the
element if found.
...
This sample search for the `.selector` CSS selector, and removes the "class1", "class2" and "class3" classes from the
element if found.
...
The screen builder is the main builder in this set. It allows the developer to generate dinamically an XML Screen
structure:
...
This builder has also a method that generates a client action ready to store the newly generated screen in the system
and access into it:
...
This method generates a [screen client action](#screen-action-builder) in the `public` menu pointing to the screen
generated within the builder.
