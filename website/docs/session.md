...
AWE, by default, uses a basic application server `HttpSession` object. So no action is required to configure this option. 
This option is useful when you deploy your application on a clustered environment and the balancer server uses the `JSESSIONID` 
cookie generated by the application server.
